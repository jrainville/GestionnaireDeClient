			...
			if(mysqli_num_rows($req)!=0){
				...
				while($enr=mysqli_fetch_assoc($req)){
					if(!in_array($enr['client_id'], $tPaiements)){
						...
						$affichage.='<tr>
							...
							<td>'.ucwords(strftime ("%m - %B",strtotime($enr['dateNaissance']))).'</td>
				...
				while($enrClients=mysqli_fetch_assoc($reqClients)){
					if(!in_array($enrClients['client_id'], $tPaiements)){
						...
						$affichage.='<tr>
							...
							<td>'.ucwords(strftime ("%m - %B",strtotime($enrClients['dateNaissance']))).'</td>
				...
				while($enr2 = mysqli_fetch_assoc($req2)){
					$affichage.='<tr>
						...
						<td>'.ucwords(strftime ("%m - %B",strtotime($enr2['dateNaissance']))).'</td>
